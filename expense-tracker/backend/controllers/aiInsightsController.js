...
const generateFallbackInsights = (
  ...
  if (savingsRate > 50) {
    ...
    insights.push({
      ...
      message: `You're saving ${savingsRate.toFixed(
        0
      )}% of your income. Try to increase it gradually.`,
    });
  } else if (savingsRate > 0) {
    insights.push({
      type: "warning",
      title: "Low Savings",
      message: `Only saving ${savingsRate.toFixed(
        0
      )}% of income. Try to reduce non-essential expenses.`,
    });
  } else {
    insights.push({
      type: "warning",
  ...
  const categories = Object.entries(categorySpending).sort(
    (a, b) => b[1] - a[1]
  );
  if (categories.length > 0) {
    const [topCategory, topAmount] = categories[0];
    const percentage =
      totalExpense > 0 ? ((topAmount / totalExpense) * 100).toFixed(0) : 0;
    insights.push({
      type: "info",
  ...
  if (totalExpense > 0) {
    insights.push({
      ...
      title: "Budget Tip",
      message:
        "Track daily expenses and set category limits to stay on target.",
    });
  } else {
    insights.push({
      type: "info",
...
export const getAIInsights = async (req, res) => {
  try {
    ...
    const prompt = `Based on this spending data, give 3 financial recommendations and  practical tips to save money(use only simple words, make it short (2 - 3 sentence) but on point)  :
  ...
  {"type":"success","title":"Good","message":"Positive feedback"}
    ...
    if (!response.ok) {
      if (data.error?.code === 429 || response.status === 429) {
        console.warn(
          "Gemini API rate limit reached, returning fallback insights"
        );
