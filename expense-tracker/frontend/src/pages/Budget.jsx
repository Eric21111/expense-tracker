...
const Budget = () => {
  ...
  const handleSubmit = (e) => {
    ...
    const config =
      categoryConfig[formData.category] || categoryConfig["Others"];
  ...
  const getBudgetStatus = (spent, limit) => {
    ...
    if (percentage >= 100)
      return { label: "Over Budget", color: "bg-red-100 text-red-600" };
    if (percentage >= 80)
      return { label: "Near Limit", color: "bg-amber-100 text-amber-600" };
    ...
    budgets.forEach((budget) => {
      ...
      if (percentage >= 80 && percentage < 100) {
        insights.push({
          type: "warning",
          title: "Budget Alert",
          message: `You're about to exceed your ${budget.category} budget. ${(
            budget.amount - spent
          ).toFixed(0)} PHP remaining.`,
        });
      } else if (percentage >= 100) {
        insights.push({
          type: "danger",
          title: "Over Budget!",
          message: `You've exceeded your ${budget.category} budget by PHP ${(
            spent - budget.amount
          ).toFixed(0)}.`,
        });
      }
          ...
          <div className="flex flex-col lg:flex-row items-start gap-6 mb-9">
            <div className="flex flex-col lg:flex-row flex-1 gap-6 w-full">
              <div className="flex-1 bg-white p-6 px-7 rounded-[20px] shadow-md relative overflow-hidden flex flex-col justify-center h-[180px] text-left">
                <h2 className="text-[30px] font-bold text-gray-900 mb-2 tracking-tight">
                  {loading
                    ? "Loading..."
                    : `PHP ${totalBudgeted.toLocaleString()}`}
                </h2>
              ...
              <div className="flex-1 bg-white p-6 px-7 rounded-[20px] shadow-md relative overflow-hidden flex flex-col justify-center h-[180px] text-left">
                <h2 className="text-[30px] font-bold text-gray-900 mb-2 tracking-tight">
                  {loading
                    ? "Loading..."
                    : `PHP ${summary.totalExpense.toLocaleString()}`}
                </h2>
            ...
            <div className="w-full lg:w-[420px] flex-shrink-0 bg-gradient-to-br from-[#144221] via-[#34A853] to-[#CCEFCC] p-7 rounded-[20px] shadow-md h-fit">
              ...
              {insights.slice(0, 2).map((insight, index) => (
                <div
                  key={index}
                  className="bg-white p-4 px-[18px] rounded-[14px] mb-3 flex gap-3.5 items-start shadow-sm"
                >
                  <div className="flex-shrink-0">
                    <img
                      src={
                        insight.type === "success" ? progressIcon : alertIcon
                      }
                      alt="Insight"
            ...
            {budgets.length === 0 ? (
              <div className="bg-white rounded-[20px] shadow-md p-12 text-center translate-y-[-240px]">
                ...
                <p className="text-gray-400 mt-4">
                  Click the + button below to add a budget
                </p>
              </div>
            ) : (
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-start gap-4">
                        <div
                          className={`w-16 h-16 rounded-2xl bg-gradient-to-br ${budget.gradient} flex items-center justify-center flex-shrink-0`}
                        >
                          <img
                            src={icon}
                            alt={budget.category}
                          ...
                          <div className="grid grid-cols-3 gap-4 mb-3">
                            <div>
                              ...
                              <p className="text-base font-bold text-gray-900">
                                PHP {budget.amount.toLocaleString()}
                              </p>
                            </div>
                            <div>
                              <p className="text-xs text-gray-500 mb-1">
              ...
              <div className="flex justify-center mb-6">
                <div className="inline-flex bg-gray-100 rounded-full p-1">
                  <button
                    type="button"
                    onClick={() =>
                      setFormData({ ...formData, budgetType: "single" })
                    }
                  ...
                  <button
                    type="button"
                    onClick={() =>
                      setFormData({ ...formData, budgetType: "multi" })
                    }
                    className={`px-6 py-2 rounded-full text-sm font-medium transition-colors ${
